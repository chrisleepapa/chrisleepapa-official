...
async function loadJamesChapterFromSupabase(chapter){
    const query=`${SUPA_URL}/rest/v1/bible_content?select=book_id,chapter,verse,content&book_id=eq.${JAMES_BOOK_ID}&chapter=eq.${chapter}&order=verse.asc`;
    const response=await fetch(query,{cache:'no-store',headers:{apikey:SUPA_KEY,Authorization:`Bearer ${SUPA_KEY}`}});
    if(!response.ok)throw new Error(`Supabase bible_content HTTP ${response.status}`);
    const rows=await response.json();
    if(!Array.isArray(rows)||rows.length===0)throw new Error(`Supabase returned no verses for James ${chapter}`);
    const normalized=rows.map(row=>({book:Number(row.book_id),chapter:Number(row.chapter),verse:Number(row.verse),content:String(row.content??'')}));
    if(!Array.isArray(globalBibleData))globalBibleData=[];
    globalBibleData=globalBibleData.filter(row=>!(Number(row.book)===JAMES_BOOK_ID&&Number(row.chapter)===chapter));
    globalBibleData.push(...normalized);
    console.info(`[Bible] James ${chapter} loaded from Supabase: ${normalized.length} verses`);
    return normalized.length;
}
function installJamesSupabaseLoader(){
    if(window.__clpJamesSupabaseLoaderInstalled)return true;
    const grid=document.getElementById('chapterGrid');
    if(!grid)return false;
    window.__clpJamesSupabaseLoaderInstalled=true;
    grid.addEventListener('click',async event=>{
        const button=event.target.closest('.chapter-btn');
        if(!button||button.dataset.supabaseReplay==='1')return;
        if(Number(currentBookId)!==JAMES_BOOK_ID)return;
        const chapter=Number(button.textContent.trim());
        if(!Number.isInteger(chapter)||chapter<JAMES_DB_CHAPTER_MIN||chapter>JAMES_DB_CHAPTER_MAX)return;
        ...
        button.textContent='…';
        try{
            await loadJamesChapterFromSupabase(chapter);
            button.dataset.supabaseReplay='1';
            button.click();
        }catch(error){
            console.error('[Bible] Supabase James chapter load failed:',error);
            alert('야고보서 본문을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.');
        }finally{
            button.disabled=false;
            button.textContent=originalLabel;
        }
    },true);
    console.info('[Bible] James 1-5 Supabase loader installed');
    return true;
}
function installBibleDataRecovery(){if(window.__clpBibleRecoveryInstalled)return true;if(typeof window.initAppAfterAuth!=='function')return false;window.__clpBibleRecoveryInstalled=true;window.initAppAfterAuth=async function(){initBibleLogout();ensureBibleAccountBar();const spinner=document.getElementById('loadingSpinner');try{await loadSupabaseData();await loadBibleDataResilient(spinner);if(spinner)spinner.style.display='none';renderBooks('ot');selectBook(1,false);setupNotifications();updateOverallProgress();installJamesSupabaseLoader()}catch(error){console.error('[Bible] data initialization failed:',error);if(spinner)spinner.innerHTML='CONNECTION FAILED.<br><small>성경 본문을 불러오지 못했습니다. 잠시 후 새로고침해 주세요.</small>'}};window.__clpBibleGateReady=true;window.dispatchEvent(new CustomEvent('clp-bible-gate-ready'));return true}
...
function loadBiblePresentationScripts(){
    if(window.__clpBiblePresentationLoaded)return;
    window.__clpBiblePresentationLoaded=true;
    ['/js/bible-creator-story.js?v=20260915','/js/bible-layout.js?v=20260915'].forEach((src,index)=>{
        const script=document.createElement('script');
        script.src=src;
        script.async=false;
        script.onload=()=>console.info('[Bible] presentation script loaded:',src);
        script.onerror=error=>console.error('[Bible] presentation script failed:',src,error);
