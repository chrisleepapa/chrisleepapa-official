...
(() => {
    ...
    const SUPA_URL='https://cvfmkcxmxkmemmshfttn.supabase.co', SUPA_KEY='sb_publishable_BbG_kRPWRFeAPvIduwPTJg_O1z_sStm';
...
@media(max-width:700px){#clp-shared-login-modal .auth-box{padding:25px}}
        ...
        const lang=(typeof window.getCurrentLang==='function'&&window.getCurrentLang()==='en')||(()=>{try{return localStorage.getItem('pref-lang')==='en'}catch(_){return false}})()?'en':'ko';
        const copy=lang==='en'?{desc:'This ID lets you use personal features such as Today, Bible, and Games.',note:'One ID lets you use personal features across Today, Bible, and Games.',initials:'INITIALS (e.g. CLP)',pin:'4-DIGIT PIN',submit:'LOGIN / CREATE'}:{desc:'이 ID 하나로 오늘의 기록, 성경, 게임 등<br>다양한 개인 기능을 이용할 수 있습니다.',note:'TODAY, BIBLE, GAME 등 개인 기능에서 같은 계정을 사용할 수 있습니다.',initials:'INITIALS (예: CLP)',pin:'4자리 PIN',submit:'LOGIN / CREATE'};
        const modal=document.createElement('div');modal.id='clp-shared-login-modal';modal.innerHTML=`<div class="auth-box"><h2>YOUR ID</h2><p>${copy.desc}</p><form autocomplete="on"><input class="auth-input" id="${prefix}-initials" maxlength="3" minlength="3" pattern="[A-Za-z]{3}" autocomplete="username" autocapitalize="characters" spellcheck="false" placeholder="${copy.initials}" required><input class="auth-input pin-input" id="${prefix}-pin" type="password" inputmode="numeric" maxlength="4" minlength="4" pattern="[0-9]{4}" autocomplete="current-password" placeholder="${copy.pin}" required><div class="auth-error" aria-live="polite"></div><button class="auth-submit" type="submit">${copy.submit}</button><p class="auth-note">${copy.note}</p></form></div>`;document.body.appendChild(modal);
        const initialsInput=modal.querySelector(`#${prefix}-initials`);
        initialsInput.addEventListener('input',()=>{const pos=initialsInput.selectionStart;initialsInput.value=normalizeInitials(initialsInput.value);try{initialsInput.setSelectionRange(pos,pos)}catch(_) {}});
        initialsInput.addEventListener('blur',()=>{initialsInput.value=normalizeInitials(initialsInput.value)});
        const form=modal.querySelector('form');form.addEventListener('submit',async e=>{e.preventDefault();const err=modal.querySelector('.auth-error');err.textContent='';initialsInput.value=normalizeInitials(initialsInput.value);const result=await login(initialsInput.value,modal.querySelector(`#${prefix}-pin`).value,true);if(!result.ok){err.textContent=result.error;return}modal.remove();if(typeof options.onSuccess==='function')options.onSuccess(result.user)});initialsInput.focus();
    }
