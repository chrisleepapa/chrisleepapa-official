...
function initScrollProgress(){
    ...
    const update = () => {
        ...
        const value = max > 0 ? (scroll / max) * 100 : 0;
...
function initMobileMenu(){
    ...
    const worksPanel = document.getElementById('mobileWorksMenu');
    ...
    const squadPanel = document.getElementById('mobileSquadMenu');

    const panels = [morePanel, worksPanel, faithPanel, squadPanel].filter(Boolean);
    const buttons = [moreBtn, worksBtn, faithBtn, squadBtn].filter(Boolean);
    ...
    [[moreBtn,morePanel],[worksBtn,worksPanel],[faithBtn,faithPanel],[squadBtn,squadPanel]].forEach(([button,panel]) => {
    ...
    document.querySelectorAll('#mobileMoreClose,#mobileSquadClose,.mobile-submenu-close').forEach(button => {
        button.addEventListener('click', event => {
            event.preventDefault();
            closeAll();
        });
...
function initDesktopDropdowns(){
    ...
    document.addEventListener('click', event => {
        if(!event.target.closest('.nav-dropdown-wrapper')) closeDesktopDropdowns();
    });
...
function getCurrentPageKey(){
    ...
    return (file.replace(/\.html$/i,'') || 'index').toLowerCase();
}
...
function initActiveNavLink(){
    ...
    if(['sistersquad','sistersquad2','sister-squad','game','gameinfo'].includes(page)){
        activateDropdownContaining(`[data-page="${page}"], [href*="sistersquad"], [href*="sister-squad"], [href*="gameinfo"]`);
    }
    ...
    const mobileMap = {
        works:['music','movie','book'],
        faith:['bible','worship']
    };
    Object.entries(mobileMap).forEach(([name,pages]) => {
        if(pages.includes(page)){
            const button = document.querySelector(`[data-mobile-menu="${name}"]`);
            if(button) button.classList.add('active');
        }
    });

    if(['gameinfo','game','journal','about','privacy','terms','sistersquad','sistersquad2','sister-squad'].includes(page)){
...
function initLangDropdown(){
    ...
    toggle.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        const open = wrapper.classList.toggle('active');
...
function setLanguage(lang){
    ...
    document.querySelectorAll('[data-i18n]').forEach(element => {
        ...
        if(i18n[currentLang][key] !== undefined) element.innerHTML = i18n[currentLang][key];
    });
    ...
    if(display) display.textContent = currentLang === 'ko' ? 'KOR' : 'ENG';
...
function initBibleLogout(){
    const buttons = document.querySelectorAll('#logoutBtn,#logoutButton,[data-action="logout"],[data-logout],.logout-btn,.logout-button');
    buttons.forEach(button => {
        ...
        button.addEventListener('click',event => {
            event.preventDefault();
            try{
                Object.keys(localStorage).filter(key => /bible|auth|user|login|session/i.test(key)).forEach(key => localStorage.removeItem(key));
                Object.keys(sessionStorage).forEach(key => sessionStorage.removeItem(key));
            }catch(_){}
            if(typeof window.onBibleLogout === 'function') window.onBibleLogout();
            else window.location.href='/bible';
        });
    });
}
...
function initSecurity(){
    ...
    document.addEventListener('keydown',event => {
        ...
        if(key==='F12' || (event.ctrlKey && event.shiftKey && (key==='I'||key==='J')) || (event.ctrlKey && key==='U')){
            event.preventDefault();
            return;
        }
...
if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded',loadComponents);
else loadComponents();
