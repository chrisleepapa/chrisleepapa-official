...
'use strict';
(() => {
// Bible owns authentication/initialization. main.js may call onMainReady on DOMContentLoaded,
// so block the legacy callback before that event to prevent a race with this gate.
window.onMainReady=function(){};
...
const JAMES_BOOK_ID=59;
const JAMES_DB_CHAPTER_MIN=1;
const JAMES_DB_CHAPTER_MAX=5;
...
function syncBibleUserContext(){
    const session=window.CLPAuth?.getUser?.()||getSession();
    if(!session?.initials)return false;
    mirrorLegacySession(session);
    if(typeof window.localUserId!=='undefined')window.localUserId=session.initials;
    if(typeof window.localUserPin!=='undefined')window.localUserPin=session.pinHash||'';
    return true;
}
...
function ensureBibleAccountBar(){
    ...
    if(!bar){
        ...
        bar.style.cssText='display:flex;align-items:center;justify-content:center;gap:10px;margin:0 auto 22px;padding:0 4px;flex-wrap:wrap;position:relative;z-index:20;';
        if(bibleContainer)bibleContainer.insertBefore(bar,bibleContainer.firstChild);
        else if(pageHeader)pageHeader.insertAdjacentElement('afterend',bar);
        else if(cloud)cloud.insertAdjacentElement('afterend',bar);
        else document.body.appendChild(bar);
    }else if(bibleContainer&&bar.parentElement!==bibleContainer){
        bibleContainer.insertBefore(bar,bibleContainer.firstChild);
    }
...
async function loadJamesChapterFromSupabase(chapter){const query=`${SUPA_URL}/rest/v1/bible_content?select=book_id,chapter,verse,content&book_id=eq.${JAMES_BOOK_ID}&chapter=eq.${chapter}&order=verse.asc`;const response=await fetch(query,{cache:'no-store',headers:{apikey:SUPA_KEY,Authorization:`Bearer ${SUPA_KEY}`}});if(!response.ok)throw new Error(`Supabase bible_content HTTP ${response.status}`);const rows=await response.json();if(!Array.isArray(rows)||rows.length===0)throw new Error(`Supabase returned no verses for James ${chapter}`);const normalized=rows.map(row=>({book:Number(row.book_id),chapter:Number(row.chapter),verse:Number(row.verse),content:String(row.content??'')}));if(!Array.isArray(globalBibleData))globalBibleData=[];globalBibleData=globalBibleData.filter(row=>!(Number(row.book)===JAMES_BOOK_ID&&Number(row.chapter)===chapter));globalBibleData.push(...normalized);console.info(`[Bible] James ${chapter} loaded from Supabase: ${normalized.length} verses`);return normalized.length}
function installJamesSupabaseLoader(){if(window.__clpJamesSupabaseLoaderInstalled)return true;const grid=document.getElementById('chapterGrid');if(!grid)return false;window.__clpJamesSupabaseLoaderInstalled=true;grid.addEventListener('click',async event=>{const button=event.target.closest('.chapter-btn');if(!button||button.dataset.supabaseReplay==='1')return;if(Number(currentBookId)!==JAMES_BOOK_ID)return;const chapter=Number(button.textContent.trim());if(!Number.isInteger(chapter)||chapter<JAMES_DB_CHAPTER_MIN||chapter>JAMES_DB_CHAPTER_MAX)return;event.preventDefault();event.stopImmediatePropagation();const originalLabel=button.textContent;button.disabled=true;button.textContent='…';try{await loadJamesChapterFromSupabase(chapter);button.dataset.supabaseReplay='1';button.click()}catch(error){console.error('[Bible] Supabase James chapter load failed:',error);alert('야고보서 본문을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.')}finally{button.disabled=false;button.textContent=originalLabel}} ,true);console.info('[Bible] James 1-5 Supabase loader installed');return true}
function installBibleReadPersistence(){
    if(window.__clpBibleReadPersistenceInstalled)return true;
    if(typeof window.saveChapterProgress!=='function'||typeof window.updateOverallProgress!=='function')return false;
    let pending=Promise.resolve();
    const originalUpdateOverallProgress=window.updateOverallProgress;
    async function reliableSave(chKey,isComplete){
        const session=window.CLPAuth?.getUser?.()||getSession();
        const userId=String(session?.initials||window.localUserId||localStorage.getItem(LEGACY_USER_KEY)||'').trim();
        if(!userId)throw new Error('Bible user context missing');
        const dbKey=`${chKey}-CH`;
        const payload={user_id:userId,verse_key:dbKey,is_read:!!isComplete};
        let lastError=null;
        for(let attempt=1;attempt<=3;attempt++){
            try{
                const response=await fetch(`${SUPA_URL}/rest/v1/bible?on_conflict=user_id,verse_key`,{method:'POST',headers:{apikey:SUPA_KEY,Authorization:`Bearer ${SUPA_KEY}`,'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(payload),keepalive:true});
    ...
    window.saveChapterProgress=function(chKey,isComplete){
        pending=pending.then(()=>reliableSave(chKey,isComplete));
        window.__clpBibleChapterSavePending=pending;
        pending.catch(error=>console.error('[Bible] chapter save failed:',error));
        return pending;
    };
    window.updateOverallProgress=function(){
        const run=(async()=>{try{await pending}catch(_){}return originalUpdateOverallProgress()})();
        window.__clpBibleProgressUpdate=run;
        return run;
    };
    window.__clpBibleReadPersistenceInstalled=true;
    console.info('[Bible] chapter read persistence installed');
    return true;
}
function installBibleDataRecovery(){if(window.__clpBibleRecoveryInstalled)return true;if(typeof window.initAppAfterAuth!=='function')return false;window.__clpBibleRecoveryInstalled=true;window.initAppAfterAuth=async function(){initBibleLogout();ensureBibleAccountBar();if(!syncBibleUserContext()){showSharedLogin();return}const spinner=document.getElementById('loadingSpinner');try{await loadSupabaseData();await loadBibleDataResilient(spinner);if(spinner)spinner.style.display='none';renderBooks('ot');selectBook(1,false);setupNotifications();updateOverallProgress();await loadLeaderboard();installJamesSupabaseLoader()}catch(error){console.error('[Bible] data initialization failed:',error);if(spinner)spinner.innerHTML='CONNECTION FAILED.<br><small>성경 본문을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</small>'}};window.__clpBibleGateReady=true;window.dispatchEvent(new CustomEvent('clp-bible-gate-ready'));return true}
...
function loadBiblePresentationScripts(){if(window.__clpBiblePresentationLoaded)return;window.__clpBiblePresentationLoaded=true;['/js/bible-creator-story.js?v=20260916','/js/bible-layout.js?v=20260916'].forEach(src=>{const script=document.createElement('script');script.src=src;script.async=false;script.onload=()=>console.info('[Bible] presentation script loaded:',src);script.onerror=error=>console.error('[Bible] presentation script failed:',src);document.head.appendChild(script)})}
async function init(){try{await loadAuth();resetLegacyClientDataOnce();window.onMainReady=function(){};addOriginalCreationContext();loadBiblePresentationScripts();ensureBibleAccountBar();const session=getSession();if(session){syncBibleUserContext();hideLegacyAuthModal();ensureBibleAccountBar()}else showSharedLogin();if(!installBibleReadPersistence())setTimeout(()=>installBibleReadPersistence(),0);if(!installBibleDataRecovery())setTimeout(()=>{if(!installBibleDataRecovery())window.dispatchEvent(new CustomEvent('clp-bible-gate-ready'));installJamesSupabaseLoader()},0);else setTimeout(installJamesSupabaseLoader,0);if(session&&typeof window.checkAuth==='function')window.checkAuth()}catch(error){console.error('[bible-auth-gate]',error)}}
window.addEventListener('chrisleepapa-auth-change',event=>{const session=event.detail?.user||getSession();ensureBibleAccountBar();if(session){syncBibleUserContext();hideLegacyAuthModal();ensureBibleAccountBar()}else{hideLegacyAuthModal();ensureBibleAccountBar()}});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();
