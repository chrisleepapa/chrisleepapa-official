...
@media(max-width:700px){#clp-shared-login-modal .auth-box{padding:25px}}
        ...
        const modal=document.createElement('div');modal.id='clp-shared-login-modal';modal.innerHTML=`<div class="auth-box"><h2>YOUR TODAY</h2><p>사이트 공통 계정으로 로그인하면<br>나만의 기록을 저장할 수 있습니다.</p><form autocomplete="on"><input class="auth-input" id="${prefix}-initials" maxlength="3" minlength="3" pattern="[A-Za-z]{3}" autocomplete="username" autocapitalize="characters" placeholder="INITIALS (예: CLP)" required><input class="auth-input pin-input" id="${prefix}-pin" type="password" inputmode="numeric" maxlength="4" minlength="4" pattern="[0-9]{4}" autocomplete="current-password" placeholder="4자리 PIN" required><div class="auth-error" aria-live="polite"></div><button class="auth-submit" type="submit">LOGIN / CREATE</button><p class="auth-note">한 번 로그인하면 성경 등 로그인 필요한 메뉴에서도 같은 계정을 사용합니다.</p></form></div>`;document.body.appendChild(modal);
