...
@media(max-width:700px){#clp-shared-login-modal .auth-box{padding:25px}}
        ...
        const modal=document.createElement('div');modal.id='clp-shared-login-modal';modal.innerHTML=`<div class="auth-box"><h2>YOUR ID</h2><p>${copy.desc}</p><form autocomplete="on"><input class="auth-input" id="${prefix}-initials" maxlength="3" minlength="3" pattern="[A-Za-z]{3}" autocomplete="username" autocapitalize="characters" placeholder="${copy.initials}" required><input class="auth-input pin-input" id="${prefix}-pin" type="password" inputmode="numeric" maxlength="4" minlength="4" pattern="[0-9]{4}" autocomplete="current-password" placeholder="${copy.pin}" required><div class="auth-error" aria-live="polite"></div><button class="auth-submit" type="submit">${copy.submit}</button><p class="auth-note">${copy.note}</p></form></div>`;document.body.appendChild(modal);
